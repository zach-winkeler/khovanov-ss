...
N = method();
N(BraidRes) := (br) -> (
	return fold((acc, subset) -> acc + subsetIdeal(br, subset), ideal(0), subsets(keys(br.adjacent)))
);

subsetIdeal = method();
subsetIdeal(BraidRes, List) := (br, vs) -> (
	AllInVars := flatten(apply(vs, (v) -> inVars(br, v)));
	AllOutVars := flatten(apply(vs, (v) -> outVars(br, v)));
	NetInVars := toList(set(AllInVars) - set(AllOutVars));
	NetOutVars := toList(set(AllOutVars) - set(AllInVars));
	inProd := fold((prod, var) -> prod * var, 1, NetInVars);
	outProd := fold((prod, var) -> prod * var, 1, NetOutVars);
	return ideal(outProd - inProd);
);
...
(b, sing) -> (
		...
		for i from 1 to 2^(#b.word)-1 do (
			crossing := changingBit(grayCode(i-1) ^^ start, grayCode(i) ^^ start);
			if (((grayCode(i-1) ^^ start) & (2^crossing)) == 0) xor (b.word#crossing < 0) then (
					splitCrossing(br, crossing);
			) else (
					joinCrossing(br, crossing);
			);
			
			key = grayCode(i) ^^ start;
