...
-- produces the subideal of N generated by non-trivial 2-cycles
N2 = method();
N2(BraidRes) := Ideal =>
(br) -> (
	out = ideal(0*br.r_0);
	for i from 0 to #(br.edges)-1 do (
		e1 := (br.edges)#i;
		e2 := ((br.adjacent)#(e1.t))#0;
		if e2.t == e1.s then (
...
LDPlus = method();
LDPlus(BraidRes, Vertex) := LabeledComplex =>
(br, v) -> (
	r := br.r;
	m0 := labeledModule(r^1, {hashTable{v => {0}}});
	m1 := labeledModule(r^1, {hashTable{v => {1}}});
	d0 := labeledModuleMap(m1, m0, map(r^1, r^1, L(br, v)));
	d1 := labeledModuleMap(m0, m1, map(r^1, r^1, LPlus(br, v)));
